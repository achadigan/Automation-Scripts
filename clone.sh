#!/bin/bash
#This script clones, sparsifys and syspreps linux guest
original=$1
destination=$2
name=$original
...
prepare () {
virt-sysprep -d $destination --operations customize --firstboot-command "touch /.autorelabel ; sudo 
hostnamectl set-hostname $name ; sudo dbus-uuidgen > /etc/machine-id ; sudo reboot"
}

sparsify () {
echo "Sparse disk optimization"
virt-sparsify --check-tmpdir ignore --compress --convert qcow2 --format qcow2 
/var/lib/libvirt/images/$destination.qcow2 /var/lib/libvirt/images/$destination.sparse
...
chown qemu:qemu /var/lib/libvirt/images/$destination.qcow2 || chown libvirt-qemu:libvirt-qemu 
/var/lib/libvirt/images/$destination.qcow2
}
