#!/bin/bash
# This script attaches a disk to a live guest
guest=$1
device=$2
...
# Checks if the device given is already in use
# and display alll the block device attached to the guest
if grep -qw "$device" <<< $(virsh domblklist $guest | tail -n +3 | head -n -1 | awk '{ print $1; }') ; 
then
